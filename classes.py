    ...
    def _to_dict(data: Union[dict, list]) -> dict:
        ...
        for data_idx, frame in enumerate(data):
            ...
            for idx, face in enumerate(list(frame)):
                ...
                rowdict.update({"count" + str(idx): face["count"]})
    ...
    def analyze(
        ...
        while self.cap.isOpened():
            ...
            try:
                ...
                faces = detector.detect_emotions(padded_frame, frameCount=frameCount)
                if detection_box is not None:
                    try:
                        for face in faces:
            ...
            if faces:
                data.append(faces)
            # else: data.append({'box':{0,0,0,0}, 'emotions': {'angry': 0.0, 'disgust': 0.0, 'fear': 0.0, 'happy': 0.0, 'sad': 0.0, 'surprise': 0.00,
            #               'neutral': 0.0}, 'count': frameCount})
